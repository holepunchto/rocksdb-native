...
module.exports = class RocksDBState extends ReadyResource {
  ...
  async _suspend() {
    ...
    this.db._logger.log('rocks suspend: draining io (inflight=' + this.io.count + ')')

    while (!this.io.isIdle()) await this.io.idle()

    this.db._logger.log('rocks suspend: io idle, calling native suspend')
    ...
    try {
      ...
      this.db._logger.log('rocks suspend: native returned, lock released')
    } finally {
      this.io.dec()
    }
