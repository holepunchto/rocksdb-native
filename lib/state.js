...
module.exports = class RocksDBState extends ReadyResource {
  ...
  async update(opts) {
    ...
    this._updating = true
    try {
      if (this._suspending) await this._suspend(opts)
  ...
  async _suspend(opts = {}) {
    ...
    await log('rocks suspend: draining io (inflight=' + this.io.count + ')')

    while (!this.io.isIdle()) await this.io.idle()

    await log('rocks suspend: io idle, calling native suspend')
    ...
    try {
      ...
      await log('rocks suspend: native returned, lock released')
    } finally {
      this.io.dec()
    }
